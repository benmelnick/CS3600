...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # determine most probable location that each ghost may be at
        probable_locations = []
        for distribution in livingGhostPositionDistributions:
            # take the most likely location for a ghost from its position belief distribution
            probable_locations.append(distribution.argMax())

        # find the closest ghost
        goal_pos = None
        goal_dis = float('inf')
        for position in probable_locations:
            ghostDis = self.distancer.getDistance(pacmanPosition, position)
            if ghostDis < goal_dis:
                goal_pos = position
                goal_dis = ghostDis

        # pick action that brings pacman as close to the closest ghost as possible
        best_action = None
        best_dist = float('inf')
        for action in legal:
            successor_pos = Actions.getSuccessor(pacmanPosition, action)
            successor_distance = self.distancer.getDistance(successor_pos, goal_pos)
