...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                all_possible[p] = emissionModel[true_distance] * self.beliefs.get(p)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        all_possible = util.Counter()
        for p in self.legalPositions:  # looking at all legal positions
            # get the position distribution of ghost starting at p
            new_pos_dist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for newPos, prob in new_pos_dist.items():
                # look at all the positions and probabilities of reaching them
                all_possible[newPos] += self.beliefs[p] * prob  # update the belief
        self.beliefs = all_possible
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for num in range(0, self.numParticles):
                jail_particles.append(self.getJailPosition())
            self.particles = jail_particles
        else:
            # create a dictionary to sample new particles from - will be used to create a weight for each particle
            # key - particle    value - number of instances of particle
            beliefs = util.Counter()
            for particle in self.particles:
                beliefs[particle] += 1

            # weigh particles
            for pos, val in beliefs.items():
                true_dist = util.manhattanDistance(pacmanPosition, pos)
                # weight(pos) = sensor_model * num of particles for pos
                beliefs[pos] = emissionModel[true_dist] * val
            ...
            if sum(beliefs.values()) == 0:
                ...
                self.particles = []
                for num in range(0, self.numParticles):
                    self.particles.append(util.sample(beliefs))  # re-sample particles based on weights
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles:
            new_post_dist = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            updated_particles.append(util.sample(new_post_dist))
    ...
    def getBeliefDistribution(self):
        ...
        for part in self.particles:
             belief_distribution[part] = belief_distribution[part] + 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        # initialize list of tuples with all possible combos of ghost positions
        ghost_tuples = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(ghost_tuples)

        counter = 0
        particles = []
        while counter < self.numParticles:
            for tuple in ghost_tuples:
                particles.append(tuple)
                counter += 1
        self.particles = particles
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        # dictionary to create a new sample of particles from
        beliefs = util.Counter()
        for particle in self.particles:
            # particle is tuple of ghost positions
            # particle weight is the sum of weighing every ghost position using the sensor model
            prob = 1.0
            for ghost_idx in range(self.numGhosts):
                if noisyDistances[ghost_idx] != None:
                    # if ghost has not been caught - calculate weight of given ghost pos
                    dist = util.manhattanDistance(particle[ghost_idx], pacmanPosition)
                    prob = prob * emissionModels[ghost_idx][dist]
                else:
                    # if caught put in jail
                    particle = self.getParticleWithGhostInJail(particle, ghost_idx)
            beliefs[particle] += prob

        beliefs.normalize()

        if beliefs.totalCount() == 0:
            # initialize particles and put all ghosts in jail
            self.initializeParticles()
            for particle in self.particles:
                for i in range(self.numGhosts):
            ...
            for num in range(self.numParticles):
                new_particles.append(util.sample(beliefs))
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for old_particle in self.particles:
            # particle is tuple of ghost positions
            new_particle = list(old_particle)
            # update each entry in list of ghost positions for current particle
            for i in range(self.numGhosts):
                new_particle[i] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, new_particle), i, self.ghostAgents[i]))
